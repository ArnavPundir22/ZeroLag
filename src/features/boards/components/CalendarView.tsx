...
export const CalendarView: React.FC = () => {
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col bg-background relative isolate">
      ...
      {/* Calendar Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-surface/30 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-accent/20 border border-accent/30 flex items-center justify-center text-accent shadow-lg">
        ...
        <div className="flex gap-2">
          <button onClick={prevMonth} className="p-2 rounded-lg bg-surface hover:bg-surface-hover border border-white/5 text-text-secondary hover:text-white transition-colors">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 text-sm font-bold rounded-lg bg-surface hover:bg-surface-hover border border-white/5 text-text-secondary hover:text-white transition-colors">
            Today
          </button>
          <button onClick={nextMonth} className="p-2 rounded-lg bg-surface hover:bg-surface-hover border border-white/5 text-text-secondary hover:text-white transition-colors">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
        <div className="bg-surface/40 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col min-h-full">
          {/* Day Names */}
          <div className="grid grid-cols-7 border-b border-white/10 bg-black/20">
            {dayNames.map(name => (
              <div key={name} className="py-3 text-center text-xs font-bold text-text-secondary uppercase tracking-widest">
                {name}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 flex-1 auto-rows-fr">
            {days.map((date, index) => {
              ...
              return (
                <div key={date.toISOString()} className={`border-r border-b border-white/5 min-h-[120px] p-2 flex flex-col gap-2 transition-colors hover:bg-white/[0.02] ${isToday ? 'bg-accent/5' : ''}`}>
                  <div className="flex items-center justify-between">
                    <span className={`text-sm font-bold w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-accent text-white shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'text-text-secondary'}`}>
                      {date.getDate()}
                    </span>
                    {dayTasks.length > 0 && (
                      <span className="text-[10px] font-bold text-text-secondary bg-black/20 px-1.5 py-0.5 rounded border border-white/5">
                        {dayTasks.length}
                      </span>
                    )}
                  </div>
                    ...
                    {dayTasks.map(task => {
                      ...
                      return (
                        <div 
                          ...
                          className={`px-2 py-1.5 rounded-lg text-xs cursor-pointer truncate transition-all shadow-sm ${
                            isDone 
